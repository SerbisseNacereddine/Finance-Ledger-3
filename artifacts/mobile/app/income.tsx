...
export default function IncomeScreen() {
  ...
  const filtered = useMemo(() => {
    let ops = filterByType(operations, 'income');
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: webPadTop + 12, backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>Income</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      >
        ...
        <View style={[styles.totalCard, { backgroundColor: COLOR + '15' }]}>
          <Text style={[styles.totalLabel, { color: COLOR }]}>Total Income</Text>
          <Text style={[styles.totalAmount, { color: COLOR }]}>{formatNumber(total)}</Text>
          <Text style={[styles.totalNote, { color: COLOR + 'AA' }]}>Total money you earned</Text>
        </View>

        <SearchBar value={search} onChange={setSearch} />

        {filtered.length === 0 ? (
          <EmptyState message="No income found" showButton={false} />
        ) : (
