...
export default function LentScreen() {
  ...
  const filtered = useMemo(() => {
    let ops = filterByType(operations, 'lent');
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: webPadTop + 12, backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>Money Lent</Text>
        <View style={{ width: 36 }} />
      </View>
      ...
      >
        ...
        <View style={[styles.totalCard, { backgroundColor: COLOR + '15' }]}>
          <Text style={[styles.totalLabel, { color: COLOR }]}>Total Money Lent</Text>
          <Text style={[styles.totalAmount, { color: COLOR }]}>{formatNumber(total)}</Text>
          <Text style={[styles.totalNote, { color: COLOR + 'AA' }]}>Money you lent to others</Text>
        </View>

        <SearchBar value={search} onChange={setSearch} />

        {filtered.length === 0 ? (
          <EmptyState message="No lent money recorded" showButton={false} />
        ) : (
